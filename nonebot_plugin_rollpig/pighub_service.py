...
PIGHUB_USER_AGENT = "RollPig/0.2 (+https://github.com/Bearlele/nonebot-plugin-rollpig)"
...
class PigHubService:
    ...
    # ================================ 缓存读写 ================================ #
    # 只缓存 PigHub API 返回的标题、文件名、图片 URL 等索引元数据。图片本体仍由
    # OneBot/QQ 按需拉取，避免插件主动镜像 PigHub 全站资源。
    ...
    async def ensure_ready(self) -> bool:
        ...
        有旧缓存时立即返回，并在过期时后台刷新；完全没有缓存时才等待一次真实请求。
        """
    ...
    def search(self, keyword: str, *, image_id: int | None = None) -> list[dict[str, Any]]:
        if image_id is not None:
            return [item for item in self.images if str(item.get("id") or "") == str(image_id)]
...
def normalize_pighub_image_item(item: Any) -> dict[str, Any] | None:
    """把 PigHub 新旧 API 条目归一成命令层使用的 title/thumbnail/filename 结构。"""
...
def build_pighub_image_url(pig_item: dict[str, Any]) -> str | None:
...
def match_pighub_keyword(pig_item: dict[str, Any], keyword: str) -> bool:
    ...
    return any(lowered in str(pig_item.get(field, "")).lower() for field in ("title", "filename"))
